...
class UserDirectoryStore(UserDirectoryBackgroundUpdateStore):
    ...
    """
     Get the User id from given Identifer 
     In Login If user put 
        : username,
        : Phone Number
        : email 
    then query will grab the username for login 

    """

    async def get_users_ID(self, identifier: str) -> Optional[str]:
        def _get_user_txt(txn: LoggingTransaction, identifier) -> Optional[str]:
            # sql = f"""  SELECT users.name FROM users INNER JOIN user_threepids ON users.name = user_threepids.user_id WHERE users.name = %s OR user_threepids.address = %s ; """

            def checkIdentifer(identifier):

                if identifier.startswith("@"):
                    sql = f""" SELECT users.name FROM users INNER JOIN user_threepids ON users.name = user_threepids.user_id WHERE users.name = %s  """
                    txn.execute(sql, [identifier])
                else:
                    sql = f""" SELECT users.name FROM users INNER JOIN user_threepids ON users.name = user_threepids.user_id WHERE user_threepids.address = %s """
                    txn.execute(sql, [identifier])
                return txn

            # txn.execute(sql, [identifier])
            row = checkIdentifer(identifier).fetchone()
            if row:
                return row[0]
            else:
                return None
        ...
        return await self.db_pool.runInteraction(
            "get_user_ID", _get_user_txt, identifier
        )
